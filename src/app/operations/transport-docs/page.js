...
export default function AWBPage() {
  ...
  const [newAWB, setNewAWB] = useState({
    doc_number: '', doc_type: 'MAWB', parent_mawb_id: '', shipment_id: '', provider_id: '',
    shipper_contact_id: '', consignee_contact_id: '', origin_location: '', destination_location: '',
    ...
    weight_charge: '', other_charges: '', status: 'Not Transmitted'
  });
  ...
  const kpis = useMemo(() => {
    ...
    state.transportDocuments.forEach(a => {
      if (a.doc_type === 'MAWB' || a.doc_type === 'Master (MAWB)') master++;
      if (a.doc_type === 'HAWB' || a.doc_type === 'House (HAWB)') house++;
      totalWeight += parseFloat(a.chargeable_weight_kg || 0);
      totalRevenue += parseFloat(a.total_charges || 0);
    });

    return {
      total: state.transportDocuments.length,
      master,
      house,
      weight: totalWeight,
  ...
  const columns = [
    { 
      ...
      render: (row) => (
        <span className={styles.awbNumberCell}>
          {formatAWBNumber(row.doc_number)}
        </span>
      ) 
    },
    { 
      ...
      accessor: 'doc_type', 
      render: (row) => (
        <Badge variant={row.doc_type === 'MAWB' || row.doc_type === 'Master (MAWB)' ? 'primary' : 'neutral'} dot>
          {row.doc_type}
        </Badge>
      ) 
    },
    ...
    { key: 'issued', label: 'ISSUED', accessor: 'issued_at', render: (row) => <span style={{ fontSize: '12px', color: '#94A3B8' }}>{row.issued_at ? formatDate(row.issued_at) : 'Draft'}</span> },
    { key: 'actions', label: '', accessor: 'actions', align: 'right',
      render: (row) => (
        <div className={styles.actionButtons}>
          <button className={styles.actionBtn} title="View Details" onClick={(e) => { e.stopPropagation(); router.push(`/operations/awb/${row.doc_id}`); }}>
            <Eye size={16} />
          </button>
  ...
  const handleCreateOrUpdate = () => {
    if (!newAWB.doc_number?.trim() || !newAWB.shipment_id || !newAWB.provider_id) return;
    ...
    const payload = { 
      ...
      status: newAWB.status || 'Not Transmitted',
      issued_by: 'user-1'
    };
    ...
    setNewAWB({ doc_number: '', doc_type: 'MAWB', parent_mawb_id: '', shipment_id: '', provider_id: '', shipper_contact_id: '', consignee_contact_id: '', origin_location: '', destination_location: '', pieces: '', gross_weight_kg: '', rate_class: 'Q', declared_value_for_carriage: '', declared_value_for_customs: '', freight_terms: 'Prepaid', currency_code: 'USD', weight_charge: '', other_charges: '', status: 'Not Transmitted' });
  };
      ...
      <div className={styles.page}>
        ...
        <div className={styles.tableContainer}>
          <DataTable
            ...
            onRowClick={(row) => router.push(`/operations/awb/${row.doc_id}`)}
            searchPlaceholder="Search by AWB number, carrier..."
            filters={[
              { key: 'doc_type', label: 'Type', options: ['MAWB', 'HAWB', 'Master (MAWB)', 'House (HAWB)', 'Direct'] },
              { key: 'status', label: 'Status', options: ['Not Transmitted', 'Sent', 'Acknowledged (FMA)', 'Rejected (FNA)', 'Issued'] }
            ]}
          />
        </div>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handleCreateOrUpdate} disabled={!newAWB.doc_number?.trim() || !newAWB.shipment_id || !newAWB.provider_id} style={{ background: '#6366F1', borderColor: '#6366F1' }}>
        ...
        <div className={styles.form}>
          <div className="form-row">
            <div className="form-group">
              ...
              <input className="form-input" value={newAWB.doc_number || ''} onChange={e => setNewAWB(p => ({ ...p, doc_number: e.target.value }))} placeholder="e.g. 157-12345675" />
            </div>
            <div className="form-group">
              <label className="form-label">AWB Type</label>
              <select className="form-select" value={newAWB.doc_type || 'MAWB'} onChange={e => setNewAWB(p => ({ ...p, doc_type: e.target.value, parent_mawb_id: e.target.value === 'MAWB' || e.target.value === 'Master (MAWB)' ? '' : p.parent_mawb_id }))}>
          ...
          {(newAWB.doc_type === 'HAWB' || newAWB.doc_type === 'House (HAWB)') && (
            <div className="form-row">
              <div className="form-group">
                ...
                <select className="form-select" value={newAWB.parent_mawb_id} onChange={e => setNewAWB(p => ({ ...p, parent_mawb_id: e.target.value }))}>
                  ...
                  {state.transportDocuments.filter(a => a.doc_type === 'Master (MAWB)' || a.doc_type === 'MAWB').map(a => <option key={a.doc_id} value={a.doc_id}>{a.doc_number}</option>)}
                </select>
              </div>
            </div>
          )}
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <select className="form-select" value={newAWB.provider_id || ''} onChange={e => setNewAWB(p => ({ ...p, provider_id: e.target.value }))}>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <select className="form-select" value={newAWB.origin_location || ''} onChange={e => setNewAWB(p => ({ ...p, origin_location: e.target.value }))}>
            ...
            <div className="form-group">
              ...
              <select className="form-select" value={newAWB.destination_location || ''} onChange={e => setNewAWB(p => ({ ...p, destination_location: e.target.value }))}>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <select className="form-select" value={newAWB.status || 'Not Transmitted'} onChange={e => setNewAWB(p => ({ ...p, status: e.target.value }))}>
