...
export default function ManifestsPage() {
    ...
    { 
      ...
      render: (row) => (
        <div className={styles.actionButtons}>
          ...
          <button className={`${styles.actionBtn} ${styles.deleteBtn}`} title="Delete" onClick={(e) => { e.stopPropagation(); setShowDeleteConfirm(row.manifest_id); }}>
            <Trash2 size={16} />
          </button>
        </div>
      )
  ...
  const handleCreate = () => {
    ...
    dispatch({ 
      type: 'CREATE_TRANSPORT_MANIFEST', 
  ...
  const handleUpdate = () => {
    ...
    dispatch({ 
      type: 'UPDATE_TRANSPORT_MANIFEST', 
  ...
  const handleDelete = (manifestId) => {
    dispatch({ type: 'DELETE_TRANSPORT_MANIFEST', payload: manifestId });
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="danger" onClick={() => handleDelete(showDeleteConfirm)}>Delete Manifest</Button>
          </>
        }
      >
